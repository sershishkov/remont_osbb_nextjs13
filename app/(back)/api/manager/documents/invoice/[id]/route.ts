...
export const GET = async (request: NextRequest, { params }: Props) => {
  ...
  try {
    await connectToDB();
    const one__ITEM = await Model__DocumentInvoice.findById(id)
      .populate({
        path: 'contract',
...
export const PUT = async (request: NextRequest, { params }: Props) => {
  ...
  try {
    ...
    const updated__ITEM = await Model__DocumentInvoice.findByIdAndUpdate(
      id,
      new__ITEM,
      {
        new: true,
...
export const DELETE = async (request: NextRequest, { params }: Props) => {
  ...
  try {
    ...
    if (currentRole === 'admin') {
      const one__ITEM = await Model__DocumentInvoice.findByIdAndDelete(id);
      ...
      await Model__DocumentInvoice.findByIdAndUpdate(
        id,
